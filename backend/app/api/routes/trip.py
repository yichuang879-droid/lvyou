"""
Trip 路由 —— HTTP 层：接收请求 → 调 service → 返回响应。
"""
from fastapi import APIRouter, HTTPException
from app.models.schemas import (
    Itinerary, TokenStatsResponse, TripDetailResponse, TripEditRequest,
    TripListResponse, TripRequest, TripSaveRequest,
)
from app.services.storage_service import (
    delete_itinerary_by_trip_id, get_itinerary_by_trip_id, get_token_stats,
    list_saved_itineraries, save_itinerary,
)
...
def save_trip(request: TripSaveRequest) -> dict[str, str]:
    """保存 itinerary，返回 trip_id。"""
    saved_id = save_itinerary(request.itinerary)
    return {"message": "Trip itinerary saved successfully.", "trip_id": saved_id}
...
def delete_trip(trip_id: str) -> dict[str, str]:
    ...
    if not delete_itinerary_by_trip_id(trip_id):
        raise HTTPException(status_code=404, detail="Trip not found.")
